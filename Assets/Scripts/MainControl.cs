...
public class MainControl : MonoBehaviour {
	...
	public void Release () {
		...
		if (GameObject.Find ("Terrain").GetComponent<NotificationCenter> ().getIdIndic () == 2)
		...
		if (Target != null) {
			...
			foreach (GameObject marker in markers) {
				if (Target.transform.position.x >= marker.transform.position.x - 0.1 && Target.transform.position.x <= marker.transform.position.x + 0.1 && Target.transform.position.z >= marker.transform.position.z - 0.1 && Target.transform.position.z <= marker.transform.position.z + 0.1) {
					...
					//move real element
					bigElement.GetComponent<Move>().setDestPos (new Vector3 (marker.transform.position.x * 38.46f, 0.45f, marker.transform.position.z * 38.46f));
					...
					StartCoroutine (WaitBeforeHorizontalRotation(bigElement));
	...
	IEnumerator NewObject (GameObject bigElement) {
		...
		if (avancement == 1) {
			...
			bigElement = getBigElementByName (mockupObjects [0].name);
		}
		else
	...
	void InstantiateObject (int index, string rsc, int rotY, int fRotY) {
		...
		obj.transform.localPosition = new Vector3 (mockupObjects [index].transform.localPosition.x, 0.09f, mockupObjects [index].transform.localPosition.z);
		obj.transform.localScale = new Vector3 (10, 7, 10);
